...
const { address, abi } = require("./abis/sushiswapv2lock3r.js");
const { getCurrentGasPrices } = require("../helper/gasGetter");

//Initialize account and abi
const account = wallet.connect(provider);
const SushiswapV2Lock33r = new ethers.Contract(address, abi, account);
...
async function main() {
    try {
        workable = await SushiswapV2Lock3r.workable();
        if (!jobTXPending && workable) {
            await UpdateGas();
            jobTXPending = true;
            const tx = await SushiswapV2Lock3r.work({
                gasPrice: gas * 1e9,
                gasLimit: 100000,
            });
