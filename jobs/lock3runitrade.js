...
const { address, abi } = require("./abis/lock3runitrade.js");
const { getCurrentGasPrices } = require("../helper/gasGetter");

//Initialize account and abi
const account = wallet.connect(provider);
const Lock3rUniTrade = new ethers.Contract(address, abi, account);
...
async function main() {
    try {
        workable = await Lock3rUniTrade.workable();
        if (!jobTXPending && workable) {
            await UpdateGas();
            jobTXPending = true;
            const tx = await Lock3rUniTrade.work({
                gasPrice: gas * 1e9,
                gasLimit: 100000,
            });
